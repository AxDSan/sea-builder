...
Promise.all([
  ...
  program
    .version("1.0.0")
    .description("Build a Single Executable Application (SEA) from a Node.js project")
    .requiredOption("-i, --input <file>", "Input file (e.g. server.ts)")
    .option("-icon, --icon <file>", "Icon file (e.g. icon.ico) [Windows only]")
    .requiredOption("-p, --platform <platform>", "Target platform (win32, linux, or macos)")
    .parse(process.argv);
 
  const showSplash = () => {
    console.log(figlet.textSync("SEA-Builder", {
      font: "slant",
      horizontalLayout: "default",
      verticalLayout: "default",
    }));
    console.log(chalk.bold.cyan("SEA-Builder") + " - Build a Single Executable Application (SEA) from a Node.js project");
    console.log("");
    console.log("Usage:");
    ...
    console.log("  -icon, --icon <file>    Icon file (e.g. icon.ico) [Windows only]");
    console.log("  -p, --platform <platform>  Target platform (win32, linux, or macos)");
    console.log("");
  ...
  async function main() {
    ...
    try {
      logger.info(chalk.bold.cyan("SEA-Builder") + " started at " + new Date());
      ...
      await injectSEABlobIntoNodeExecutable(executableOutputPath, seaConfig.output);
      logger.info(chalk.green("SEA blob injected successfully"));
 
      // Step 6: Finished
      logger.info(chalk.bold.cyan("SEA-Builder") + " finished at " + new Date());
    } catch (error) {
  ...
  async function injectSEABlobIntoNodeExecutable(nodeExecutable, seaBlobPath) {
    ...
    if (!fs.existsSync(seaBlobPath)) {
      throw new Error(`The SEA preparation blob file does not exist: ${seaBlobPath}`);
    }
